...
import http.routes.{BrandRoutes, CategoryRoutes, ShopRoutes, UserRoutes, ItemRoutes}
...
sealed abstract class HttpApi(implicit logger: Logger[IO], services: Services[IO]) {
  ...
  private val userRoutes: HttpRoutes[IO] = UserRoutes(services.users, logger).routes
  ...
  private val itemRoutes: HttpRoutes[IO] = ItemRoutes(services.items, logger).routes
  ...
  private val allRoutes = userRoutes <+> brandRoutes <+> categoryRoutes <+> shopRoutes <+> itemRoutes
